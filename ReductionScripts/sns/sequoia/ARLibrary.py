...
import glob, filecmp, shutil, datetime
from numpy import *
from string import *
...
def check_newer_script(instrument, folder):
    ...
    This was modified from the function Andrei implemented in reduce_CNCS.py
    """
...
def write_readme(folder, types):
    out = os.path.join(folder, 'README.md')
    if os.path.exists(out): return
    lines = ['# README', '']
    lines += ['## Files', '']
    for type in types:
        lines.append('* ' + output_file_types[type])
        continue
    lines.append('')
    open(out, 'wt').write('\n'.join(lines))
    return
readme_fn = "README.md"
output_file_types = dict(
    ar_script = 'reduce_{instrument}_{timestamp}.py: auto-reduction scripts with time stamps',
    dE_event_nxs = '{instrument}_{run_number}_autoreduced.nxs: event-based nexus file with tof axis transformed to delta E',
    dE_nxspe = '{instrument}_{run_number}_autoreduced.nxspe: inelastic NXSPE',
    powder_nxspe = 'powder/{instrument}_{run_number}_autoreduced_powder.nxspe: powder NXSPE',
    elastic_nxspe = 'elastic/{instrument}_{run_number}_elastic.nxspe: elastic NXSPE',
    log = "experiment_log.csv: reduction log",
    )
