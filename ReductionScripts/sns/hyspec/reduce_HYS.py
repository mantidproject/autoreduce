...
plotParameters.append({'Name':'elastic',
                       'Axis0':'Q1',
                       'Axis0_min':-2.5,
                       'Axis0_max':2.5,
                       'Axis0_Nsteps':200,
...
plotParameters.append({'Name':'5_meV',
                       'Axis0':'Q1',
                       'Axis0_min':-2.5,
                       'Axis0_max':2.5,
                       'Axis0_Nsteps':200,
...
def do_reduction(filename,output_dir):
    ...
    try:
        ...
        sys.path.insert(0,'/SNS/users/3y9/autoreduce_test')
        from publish_plot import plot_heatmap, publish_plot
            ...
            for p in plotParameters:
                ...
                try:
                    ...
                    SCplots.append(plot_heatmap(run_number, x.tolist(), y.tolist(), Zm.tolist(), 
                                                ...
                                                x_log=False, y_log=False, instrument='HYS', title=name, publish=False))
                    plot_html+="<div>{0}</div>\n".format(SCplots[-1])
                except Exception as e:
